...
import numpy as np
from foundation.stunet.architecture.initialization import InitWeights_He
from foundation.stunet.architecture.neural_network import SegmentationNetwork
import torch
...
class STUNetFID(STUNet):
    def __init__(self, input_channels, num_classes, middle=True, depth=[1,1,1,1,1,1], dims=[32, 64, 128, 256, 512, 512],
                 pool_op_kernel_sizes=None, conv_kernel_sizes=None):
        super().__init__(input_channels, num_classes, depth=depth, dims=dims,
                 pool_op_kernel_sizes=pool_op_kernel_sizes, conv_kernel_sizes=conv_kernel_sizes)

        self.middle = middle

        # Adding GAP as last layer
        self.feature_output = nn.Sequential(nn.AdaptiveAvgPool3d(1), nn.Flatten())
