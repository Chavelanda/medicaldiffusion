...
from foundation.misfm.transformer import BasicLayer
...
class PCTNetFID(PCTNet):
    def __init__(self, params, middle=True):
    ...
    def forward(self, x):
        _, _, x = self.embeddings[self.resolution_mode](x)
        x  = self.pyramid_ct.forward_fid(x)

        out = self.feature_output(x)
