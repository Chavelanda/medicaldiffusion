...
from ddpm.patch_embed3d import PatchEmbed
...
class DiT(nn.Module):
    ...
    def __init__(
        self,
        input_size: Union[int, Tuple[int, int, int]]=(32, 32, 32),
        patch_size=2,
        in_channels=4,
        hidden_size=1152,
        depth=28,
    ...
    def unpatchify(self, x):
        ...
        d, h, w = (s // p for s in self.input_size)
    ...
    def forward(self, x, t, y):
        ...
        x = self.x_embedder.unpatchify(x)                   # (N, out_channels, H, W)
        return x
...
def get_3d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0):
    """
    grid_size: tuple of the grid depth height and width
    return:
    ...
    grid = np.stack(grid, axis=0) # Shape: (3, grid_size, grid_size, grid_size)
...
# CHANGED FROM 1024 TO 1020 TO BE ABLE TO SPLIT EMBEDDING IN THREE EQUAL PARTS TO APPLY POSITIONAL EMBEDDING 
# AND NUM_HEADS FROM 16 TO 15 BECAUSE HIDDEN_SIZE MUST BE DIVISIBLE BY NUM_HEADS
def DiT_L_2(**kwargs):
    return DiT(depth=24, hidden_size=1020, patch_size=2, num_heads=15, **kwargs) 

def DiT_L_4(**kwargs):
    return DiT(depth=24, hidden_size=1020, patch_size=4, num_heads=15, **kwargs)

def DiT_L_8(**kwargs):
    return DiT(depth=24, hidden_size=1020, patch_size=8, num_heads=15, **kwargs)
...
if __name__ == '__main__':
    ...
    in_channels = 8
    device='cuda:0'
    model = DiT_S_4(input_size=input_size, in_channels=in_channels, num_classes=5, learn_sigma=False).to(device)
    model.eval()

    x = torch.randn((2, in_channels, *input_size)).to(device)
    t = torch.tensor([20., 50.]).to(device)
    y = torch.tensor([0, 3]).to(device)
